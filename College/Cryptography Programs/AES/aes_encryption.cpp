#include <string>
#include <iostream>
#include <cstdint>

using namespace std;

uint8_t gfMul(uint8_t a, uint8_t b)
{
    uint8_t result = 0;
    while (b)
    {
        if (b & 1)
            result ^= a;
        bool high_bit_set = (a & 0x80);
        a <<= 1;
        if (high_bit_set)
            a ^= 0x1B; // irreducible polynomial reduction step x^8 + x^4 + x^3 + x + 1
        b >>= 1;
    }
    return result;
}

int multiplyRC(int row, int col, int mat2[4][4])
{
    int result = 0;

    // constant matrix ...
    int mat1[4][4] = {
        {0x02, 0x03, 0x01, 0x01},
        {0x01, 0x02, 0x03, 0x01},
        {0x01, 0x01, 0x02, 0x03},
        {0x03, 0x01, 0x01, 0x02}};

    for (int k = 0; k < 4; k++)
    {
        result ^= gfMul(mat1[row][k], mat2[k][col]);
    }

    return result;
}

void rowShifting(int row[4], int times)
{
    while (times--)
    {
        for (int i = 0; i < 3; i++)
        {
            swap(row[i], row[i + 1]);
        }
    }
}

void printCipherTextRow(int mat[4][4])
{
    const char *hex = "0123456789ABCDEF";

    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            int x = mat[j][i];

            cout << hex[(x >> 4) & 0xF]
                 << hex[x & 0xF]
                 << " ";
        }
    }
}

void printHexMatrix(int mat[4][4])
{
    const char *hex = "0123456789ABCDEF";

    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            int x = mat[i][j];

            cout << hex[(x >> 4) & 0xF]
                 << hex[x & 0xF]
                 << " ";
        }
        cout << endl;
    }
}

void inputHex(int mat[4][4], string inp)
{
    int in = 0;
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            mat[j][i] = stoi(inp.substr(in, 2), nullptr, 16); // hex digit
            in += 2;
        }
    }
}

int subByteTransform(int cell)
{
    int subByteTable[16][16] = {
        {0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76},
        {0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0},
        {0xB7, 0xFD, 0x93, 0x26, 0x36, 0x3F, 0xF7, 0xCC, 0x34, 0xA5, 0xE5, 0xF1, 0x71, 0xD8, 0x31, 0x15},
        {0x04, 0xC7, 0x23, 0xC3, 0x18, 0x96, 0x05, 0x9A, 0x07, 0x12, 0x80, 0xE2, 0xEB, 0x27, 0xB2, 0x75},
        {0x09, 0x83, 0x2C, 0x1A, 0x1B, 0x6E, 0x5A, 0xA0, 0x52, 0x3B, 0xD6, 0xB3, 0x29, 0xE3, 0x2F, 0x84},
        {0x53, 0xD1, 0x00, 0xED, 0x20, 0xFC, 0xB1, 0x5B, 0x6A, 0xCB, 0xBE, 0x39, 0x4A, 0x4C, 0x58, 0xCF},
        {0xD0, 0xEF, 0xAA, 0xFB, 0x43, 0x4D, 0x33, 0x85, 0x45, 0xF9, 0x02, 0x7F, 0x50, 0x3C, 0x9F, 0xA8},
        {0x51, 0xA3, 0x40, 0x8F, 0x92, 0x9D, 0x38, 0xF5, 0xBC, 0xB6, 0xDA, 0x21, 0x10, 0xFF, 0xF3, 0xD2},
        {0xCD, 0x0C, 0x13, 0xEC, 0x5F, 0x97, 0x44, 0x17, 0xC4, 0xA7, 0x7E, 0x3D, 0x64, 0x5D, 0x19, 0x73},
        {0x60, 0x81, 0x4F, 0xDC, 0x22, 0x2A, 0x90, 0x88, 0x46, 0xEE, 0xB8, 0x14, 0xDE, 0x5E, 0x0B, 0xDB},
        {0xE0, 0x32, 0x3A, 0x0A, 0x49, 0x06, 0x24, 0x5C, 0xC2, 0xD3, 0xAC, 0x62, 0x91, 0x95, 0xE4, 0x79},
        {0xE7, 0xC8, 0x37, 0x6D, 0x8D, 0xD5, 0x4E, 0xA9, 0x6C, 0x56, 0xF4, 0xEA, 0x65, 0x7A, 0xAE, 0x08},
        {0xBA, 0x78, 0x25, 0x2E, 0x1C, 0xA6, 0xB4, 0xC6, 0xE8, 0xDD, 0x74, 0x1F, 0x4B, 0xBD, 0x8B, 0x8A},
        {0x70, 0x3E, 0xB5, 0x66, 0x48, 0x03, 0xF6, 0x0E, 0x61, 0x35, 0x57, 0xB9, 0x86, 0xC1, 0x1D, 0x9E},
        {0xE1, 0xF8, 0x98, 0x11, 0x69, 0xD9, 0x8E, 0x94, 0x9B, 0x1E, 0x87, 0xE9, 0xCE, 0x55, 0x28, 0xDF},
        {0x8C, 0xA1, 0x89, 0x0D, 0xBF, 0xE6, 0x42, 0x68, 0x41, 0x99, 0x2D, 0x0F, 0xB0, 0x54, 0xBB, 0x16}};

    int row = (cell >> 4) & 0x0F;
    int col = cell & 0x0F;
    return subByteTable[row][col];
}

int main()
{

    // inputting plain text from the user
    // then converting each 2-hex digits to a normal decimal number and storing in one cell of the hex matrix in column wise manner
    // also printing each state matrix for data validity checking

    string plainText;
    cout << "Enter the plain text in hexadecimal format without the \"0x\" part : ";
    cin >> plainText;
    int pt_state_matrix[4][4];
    inputHex(pt_state_matrix, plainText);
    // cout << "plaintext state matrix : " << endl;
    // printHexMatrix(pt_state_matrix);

    string key;
    cout << "Enter the key in hexadecimal format without the \"0x\" part : ";
    cin >> key;
    int key_state_matrix[4][4];
    inputHex(key_state_matrix, key);
    // cout << "key state matrix : " << endl;
    // printHexMatrix(key_state_matrix);

    // step 1 - pre-processing xor of pt and key state matrix

    int preProcessOutput[4][4];
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            preProcessOutput[i][j] = pt_state_matrix[i][j] ^ key_state_matrix[i][j];
        }
    }

    // cout << "output of pre-process step : " << endl;
    // printHexMatrix(preProcessOutput);

    // step 2 - sub byte transformation
    int subByteOutput[4][4];
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            subByteOutput[i][j] = subByteTransform(preProcessOutput[i][j]);
        }
    }

    // cout << "output of sub-byte transformation step : " << endl;
    // printHexMatrix(subByteOutput);

    // step 3 row shifting operation
    int shiftRowOutput[4][4];
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            shiftRowOutput[i][j] = subByteOutput[i][j];
        }
    }
    for (int i = 0; i < 4; i++)
    {
        rowShifting(shiftRowOutput[i], i);
    }

    // cout << "output of row-shifting transformation step : " << endl;
    // printHexMatrix(shiftRowOutput);

    // column mixing step...
    int columnMixingOutput[4][4];

    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            columnMixingOutput[j][i] = multiplyRC(j, i, shiftRowOutput);
        }
    }
    // cout << "output of column mixing step : " << endl;
    // printHexMatrix(columnMixingOutput);
    cout << "Cipher text after encryption process : ";
    printCipherTextRow(columnMixingOutput);
    return 0;
}
